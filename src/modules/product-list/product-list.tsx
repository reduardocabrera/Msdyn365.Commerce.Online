...
import { ProductPrice, ProductSearchResult, ProductComponent } from '@msdyn365-commerce/retail-proxy';
import {
    ArrayExtensions,
...
export interface IProductListViewProps extends IProductListProps<IProductListData> {
 ...
 */
class ProductList extends React.PureComponent<IProductListProps<IProductListData>, IProductComponentState> {
    private readonly telemetryContent: ITelemetryContent;

    private readonly productPrices: ProductPrice[];

    constructor(props: IProductListProps<IProductListData>) {
        super(props);
        this.productPrices = [];
    ...
    public render(): JSX.Element | null {
        ...
        const productCollectionViewProps = {
            ...
            GridComponentProps: {
                tag: 'table' as NodeTag, //Cambio
                className: 'table table-bordered table-striped' //Cambio
            },
            isCarousel: false, //cambio
        ...
        //return this.props.renderView(productCollectionViewProps) as React.ReactElement; cambio
        //console.log(this.props);
        return (
            <div className='table-responsive'>
                {headingComponent}
                <table className='table table-bordered table-striped' id='InventoryChecking'>
                    ...
                    <tbody>{ArrayExtensions.hasElements(updatedProducts) && updatedProducts}</tbody>
                </table>
                {productCollectionViewProps.seeAllButton}
            </div>
        );
    }
    ...
    private readonly _getProduct = (
        ...
        const Availability = productComponent ? productComponent.Quantity : 0;
        const SKU = product.ItemId;
        const Size = '3.94x5.6';
        const SizeInch = '2.83x4.5';
        const PackageType = 'vPack w/film';
        const PackageWeight = '4010lb';
        const CustomerPartNumber = '563894127';
        const Coating = 'COATING';
        const Color = 'FF56D8';
        const Thickness = '3.94';
        const ThicknessInch = '2.83';

        /* NUEVO */

        return (
            /*ProductContainer: {
                tag: 'tr' as NodeTag, //cambio
            ...
            <tr id={SKU}>
                <td>
                    <button id={SKU}>
                        <img src='https://images-us-prod.cms.commerce.dynamics.com/cms/api/sxmlvkssmq/imageFileData/MA2tuc?ver=3442' />
                    </button>
                    {Name} <button>See alternatives </button>
                </td>
                <td>
                    <input type='number' id={SKU} placeholder='Quantity' />
                </td>
                <td>{Availability}</td>
                <td>{SKU}</td>
                <td>{Size}</td>
